...
!  multisphere-scattering Fortran code gmm01s.f
!  the scattering formulations and numerical techniques used as well as
!  the usage are exactly the same as the gmm01f.f code
!  the only difference between this code and gmm01f.f is that this code
!  is less computer-memory-demanding
!  This code can calculate aggregates of a larger number of spheres
!  than the gmm01f.f code for the same available amount of computer
!  memory. Consequently, this gmm01s.f code is slower in computing
!  speed than the gmm01f.f code.
!  Yu-lin Xu
!  released to public  8/2001
!  available online at http://www.astro.ufl.edu/~xu
!  last updated  5/2003
...
!  an example of "gmm01f.par":   parameter (nLp=10000,np=20)


!  ******** Computer memory required by this code is at the level of
!                           np^4 + nLp*np^2
!  which is significantly less than (nLp**2*np**3 + np**4) for the code
!  gmm01f.f, especially for a large number of component particles. This
!  code is written in double precision arithmetic. An individual sphere
!  may reach a size parameter way beyond ~200. There is no limit to the
!  overall dimension of an aggregate.
!  Note that there may be a convergence problem for aggregates of very
!  large sizes or a very large number of component particles. *********

!----------------------------------------------------------------------

!  Revision history:

!  September 2001: A bug in subroutine transs.f was found and fixed
!                    by Zhifang Lin.
!  October 2001:   One major and two minor bugs in the calculation of
!                    internal field distributions were found and fixed.
!                    These involve subroutine internd.f.
!  November 2001:  In solving boundary condition linear equations for
...
!             The format of output for the amplitude and Mueller
...
!  the incident plane wave defines the positive z-axis, the x-z plane is
...
!  *** In addition to the output file of mueller2 for the scattering
...
!  setting factor1,factor2<1 may help to converge to a numerical
!  solution. When MXINT is exceeded, it will automatically switch to
!  BI-CGSTAB.
...
!  by the Wiscombe's criterion, which can be negative or positive in
!  the range of [-9,99]
!  Normally, set NADD=0
!-----------------------------------------------------------------------
    write(6,'(a41,i3)') &
...
!  spheres is defined by f=(r_i+r_j)/d_{ij}, where (r_i,r_j) are the
!  radii of the two spheres and d_{ij} is the separation distance of the
!  two sphere-centers. When f<fint, interaction between the pair is
!  considered to be negligible and no interaction will be calculated
!  between the two spheres. When fint=0, no sphere is excluded in
...
!------------------------------------------------------------------------
    ...
    write(file_id, '(i5.5)') nL-1
    fileout = 'gmm01s_'//trim(adjustl(file_id))// '.out'
    fileout1=fileout
    ...
    !  Phys. Rev. E 60, 2347 (1999)]
        ...
        do j=1,nmax(i)
            ...
            temp1=an(j)
            temp2=bn(j)
            if(j == 1 .OR. j == nmax(i)) write(6,'(i10,4e15.7)') &
    ...
    do ibeta=1,nbeta
        do iphai=1,nphaic
            ...
            do ithet=1,nthetc
                ...
                !  factor1,factor2=0: BI-CGSTAB [see H.A. van der Vorst, SIAM,
                !  J. Sci. Stat. Comput. 13, 631 (1992)]
...
!  computing three-dimensional internal electric field distributions for
!  each component spheres [see Xu et al. Physical Review E 60:2347 (1999)]
